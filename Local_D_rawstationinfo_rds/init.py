...
def init():
    ...
    logger = logging.getLogger('D_rawStationInfo')
    logger.setLevel(logging.DEBUG)

    fileHandler = logging.FileHandler('./pylog/D_rawStationInfo.log')
...
# ---------->> 1단계 : 공공데이터를 받아서 로컬 aws rds파일에 저장하는 작업
def step_one():
    s_time = time.time()
    ...
    sql = "CREATE TABLE elecgr_info " \
          ...
          "powertype VARCHAR(100) NOT NULL, " \
          "output INT NOT NULL, " \
    ...
    for i in range(1, count):
        ...
        for item in data:
            ...
            powertype = item.find('powertype').text
            ...
            temp.append([statnm, statid, chgerid, chgertype, addr, lat, lng, usetime,
                         busiid, businm, busicall, stat, statupddt, powertype,
                         output, method, zcode, parkingfree, note, limityn, limitdetail, delyn, deldetail])

        print("------> sub2")

        sql = "INSERT INTO elecgr_info " \
              "(statnm, statid, chgerid, chgertype, addr, lat, lng, usetime, " \
...
def step_five():
    ...
    print(
        "--- 5단계 작업 %s 초 소요됨. elecgr-info table 업데이트 동안 누적된 충전소 상태들 최신 업데이트 완료! ---" % (time.time() - start_time_step))
    print("--- 누적 %s 초 소요됨. ---" % (time.time() - start_time), "다음작업 진행중..")
